...
use std::fs::{self, File, write, metadata, read_to_string};
use std::path::{Path, PathBuf};
use serde_json::{self, Value, Result, json};
...
pub fn get_properties_path() -> String{
    let project_path = env::var(PROJECT_VAR_NAME).unwrap_or_else(|_| String::from("."));
    let path_buf: PathBuf = [project_path, String::from("data"), PROPERTIES_FILENAME.to_string()].iter().collect();
    let properties_path = path_buf.display().to_string();
    let path_str = get_path(&properties_path);  //Creates file if it does not exist already
    match metadata(&path_str){
        Ok(md) => {
            if md.len() == 0 {
                let properties = json!({"test_mode": 0});
                let properties_str = serde_json::to_string_pretty(&properties);
                write_to_file(properties_path.to_string(), properties_str.expect("Initial settings could not be created."));
            }
        },
    ...
    match load_properties(){
        Ok(data) => {
            ...
            *properties.get_mut("test_mode").unwrap() = json!(enabled);
            let properties_str = serde_json::to_string_pretty(&properties);
            write_to_file(get_properties_path(), properties_str.expect("Test mode properties could not be created."));
        },
        Err(e) => eprintln!("Error: {}", e)
    }
}
