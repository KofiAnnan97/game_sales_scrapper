...
pub fn get_simple_prices_str(store_name: &str, sales: Vec<SaleInfo>) -> String {
    ...
    sales.iter().for_each(|game| {
        prices_str.push_str(&format!(
            "\n\t- {} : {} -> {} ({}% off)",
            game.title, game.original_price, game.current_price, game.discount_percentage
        ));
    });
...
pub fn check_prices_for_display(store_sales: &[StoreSale]) -> HashMap<GameStore, Vec<usize>> {
...
pub async fn check_prices_for_email(use_html: bool) -> Result<String, String> {
    let thresholds = thresholds::load_thresholds().unwrap_or_default();
    ...
    while let Some(elem) = iter.next() {
        if elem.steam_id != 0 {
            if steam_app_ids.len() >= 10 || iter.peek().is_none() {
    ...
    if !steam_sales.is_empty() {
        ...
        if use_html {
            output.push_str(&email::create_store_cards(store_name, steam_sales));
        } else {
            output.push_str(&get_simple_prices_str(store_name, steam_sales));
        }
    }
    if !gog_sales.is_empty() {
        let store_name = GameStore::GOOD_OLD_GAMES.get_name();
        if use_html {
            output.push_str(&email::create_store_cards(store_name, gog_sales));
        } else {
            output.push_str(&get_simple_prices_str(store_name, gog_sales));
        }
    }
    if !microsoft_store_sales.is_empty() {
        let store_name = GameStore::MICROSOFT_STORE_PC.get_name();
        if use_html {
            output.push_str(&email::create_store_cards(
                store_name,
                microsoft_store_sales,
            ));
        } else {
            output.push_str(&get_simple_prices_str(store_name, microsoft_store_sales));
        }
    }

    if output.is_empty() {
        Ok(String::from(
            "No thresholded games are currently below desired price.",
        ))
    } else {
        Ok(output)
    }
...
pub fn compare_prices(store_sales: &[StoreSale]) -> Vec<SaleInfoCompare> {
    let mut compared_sales: HashMap<String, SaleInfoCompare> = HashMap::new();
    for sale in store_sales.iter() {
        let cmp_id = if !sale.alias.is_empty() {
            sale.alias.clone()
        } else {
            sale.info.title.clone()
        };
        let entry = compared_sales
            .entry(cmp_id)
        ...
        let price = sale.info.current_price;
        match sale.store {
