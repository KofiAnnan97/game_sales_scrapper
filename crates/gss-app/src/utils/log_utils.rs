...
impl Logger {
    ...
    pub fn debug(&mut self, screen: Screen, msg: &str) {
        self.batch
            .push(self.message_builder(msg, screen, LogLevel::DEBUG));
        self.batch_limit_exceeded_flush();
    }

    pub fn warn(&mut self, screen: Screen, msg: &str) {
        self.batch
            .push(self.message_builder(msg, screen, LogLevel::WARN));
        self.batch_limit_exceeded_flush();
    }

    pub fn info(&mut self, screen: Screen, msg: &str) {
        self.batch
            .push(self.message_builder(msg, screen, LogLevel::INFO));
        self.batch_limit_exceeded_flush();
    }

    pub fn error(&mut self, screen: Screen, msg: &str) {
        self.batch
            .push(self.message_builder(msg, screen, LogLevel::ERROR));
        self.batch_limit_exceeded_flush();
    }
...
pub fn get_log_path() -> String {
    let path_buf: PathBuf = [properties::get_log_path(), APP_SUBDIR.to_string()]
        .iter()
        .collect();
    let app_logs_path = path_buf.display().to_string();
    general::create_dir(&app_logs_path);
    app_logs_path
}
...
pub fn get_app_logs() -> Vec<String> {
    ...
    if let Ok(dir) = fs::read_dir(get_log_path()) {
        for d in dir {
                ...
                && entry.file_type().unwrap().is_file()
            {
                ...
                if let Some(file_name) = file_name.to_str()
                    && is_valid_log_filename(file_name)
                {
...
pub fn parse_logs(log_str: &str) -> Vec<LogData> {
    ...
    for line in log_str.lines() {
        if let Ok(data) = from_str::<LogData>(line.trim()) {
            log_data.push(data)
        }
    }
    log_data
