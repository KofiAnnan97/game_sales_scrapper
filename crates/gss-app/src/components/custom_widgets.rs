...
use iced::widget::{Container, Image, Text, button, column, container, row, slider, table, text};
use iced::{Alignment, Background, Color, Renderer, Theme, alignment::Horizontal};
use iced::{Alignment::Center, Element, Length, alignment};
...
pub fn closable_window_button(
    ...
    let content = if is_clicked {
        ...
        row![label_button].align_y(alignment::Vertical::Center)
    };
...
fn colored_badge<M: Clone + 'static>(
    value: String,
    font_size: u32,
    color: Color,
) -> iced::Element<'static, M> {
    container(text(value).size(font_size).color(Color::WHITE))
        .padding([6, 12])
...
fn price_change<'a, M: Clone + 'static>(old_price: f64, new_price: f64) -> Element<'a, M> {
    row![
        text::Rich::with_spans([text::Span::<()>::new(old_price)
            .size(16)
...
fn store_link<'a, M: Clone + 'static>(
    label: &'a str,
    id: &'a str,
    url: String,
    on_link_click: impl Fn(String, String) -> M + 'static,
) -> iced::Element<'a, M> {
    text::Rich::<String, M, Theme, Renderer>::with_spans([text::Span::new(label).link(&url)])
...
pub fn _create_sales_table<'a>(sales_info: &'a Vec<SaleInfo>) -> Table<'a, Message> {
    let columns = [
        ...
        table::column(bold_text("MSRP"), |game: &SaleInfo| {
            text(game.original_price).align_x(Center)
        })
        .width(Length::FillPortion(1)),
        table::column(bold_text("Current Price"), |game: &SaleInfo| {
            text(game.current_price).align_x(Center)
        })
        .width(Length::FillPortion(1)),
        table::column(bold_text("Discount %"), |game: &SaleInfo| {
            text(&game.discount_percentage).align_x(Center)
        })
        .width(Length::FillPortion(1)),
    ];
...
pub fn game_store_card<'a, M: Clone + 'static>(
    copied_link: &'a Option<String>,
    data: &'a StoreSale,
    on_link_click: impl Fn(String, String) -> M + 'static,
) -> Container<'a, M> {
    ...
    let handler = data
        .icon_handler
        .clone()
        .unwrap_or_else(|| Handle::from_bytes(vec![]));
    container(
        row![
            container(Image::new(handler).height(100))
                .align_y(Alignment::Center)
                .padding(10),
            column![
    ...
    .width(Length::Fill)
    .padding(10)
    .style(|_| container::Style::from(Color::from_rgb8(45, 76, 105)))
}

pub fn store_price_cell<M: Clone + 'static>(
    price: &Option<f64>,
    is_best: bool,
) -> Container<'_, M> {
    let price_str;
    ...
    container(text(price_str.clone()))
...
pub fn game_comparison_row<M: Clone + 'static>(
    data: &SaleInfoCompare,
    idx: usize,
) -> Container<'_, M> {
    ...
    let mut steam_lowest = data
        .lowest_price_stores
        .contains(&String::from(STEAM_STORE_ID));
    let mut gog_lowest = data
        .lowest_price_stores
        .contains(&String::from(GOG_STORE_ID));
    let mut ms_lowest = data
        .lowest_price_stores
        .contains(&String::from(MICROSOFT_STORE_ID));
    ...
    sale_avaiable_count += if data.microsoft_store_price.is_some() {
        1
    } else {
        0
    };
    ...
    container(
        row![
            ...
            .align_y(Alignment::Center)
            .width(Length::FillPortion(2)),
            store_price_cell(&data.steam_price, steam_lowest).width(Length::FillPortion(1)),
            store_price_cell(&data.gog_price, gog_lowest).width(Length::FillPortion(1)),
            store_price_cell(&data.microsoft_store_price, ms_lowest).width(Length::FillPortion(1)),
        ]
        .spacing(20)
        .padding(12)
        .align_y(Alignment::Center),
...
pub fn message_dialog<'a, M: Clone + 'static>(
    title: &'a str,
    body: &'a str,
    message: M,
) -> Container<'a, M> {
    container(
        column![
            ...
            column![button("OK").padding([8, 24]).on_press(message),]
                .width(Length::Fill)
    ...
    F: Fn(usize) -> M + 'static,
{
    ...
    let labels = row(choices
        .iter()
        .map(|option| {
            container(text(option.clone()).size(15))
                .width(Length::FillPortion(1))
                .center_x(Length::Fill)
                .into()
        })
    ...
    column![
        ...
        container(slider).center_x(Length::Fill).padding([2, 0]),
        labels,
    ]
    .spacing(4)
    .width(Length::Fixed(max_width))
...
pub fn text_loading_indicator<'a>(
    description: &str,
    current_frame: usize,
    frame_size: usize,
) -> Text<'a, Theme> {
