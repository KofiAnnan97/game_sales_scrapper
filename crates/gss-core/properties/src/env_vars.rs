...
use constants::operations::properties::{
    CONFIG_DIR, DECRYPT_FILENAME, DEFAULT_TEST_DIR, ENV_FILENAME, PROJECT_PATH_ENV,
    RECIPIENT_EMAIL_ENV, SMTP_EMAIL_ENV, SMTP_HOST_ENV, SMTP_PASSWORD_ENV, SMTP_PORT_ENV,
    SMTP_USERNAME_ENV, STEAM_API_KEY_ENV, TEST_PATH_ENV,
};
use files::general;
...
pub fn get_decrypt_key(project_path: String) -> String {
    ...
    if !path_buf.is_file() {
        ...
        match metadata(&path_buf) {
            ...
            Err(e) => eprintln!(
                "Could not create decrypt key file: {}\n{}",
                DECRYPT_FILENAME, e
            ),
        }
    } else {
        key_str = fs::read_to_string(path_str).unwrap_or_default();
    }
    key_str
}
    ...
    match file {
        Some(filename) => {
            if cfg!(target_os = "windows") {
                env_windows::from_filename_override(filename).ok();
            } else if cfg!(target_os = "linux") {
                env_linux::from_filename(filename).ok();
            }
        }
        None => {
            if cfg!(target_os = "windows") {
                env_windows::dotenv().ok();
            } else if cfg!(target_os = "linux") {
                env_linux::dotenv().ok();
            }
        }
...
pub fn get_variables() -> HashMap<String, EnvVar> {
    ...
    if env_path.is_file() {
        ...
        let steam_key_encrypted = Password::new(
            get_decrypt_key(get_project_path()).as_str(),
            steam_key_plain,
        );
        ...
        let smtp_pwd_encrypted =
            Password::new(get_decrypt_key(get_project_path()).as_str(), smtp_pwd_plain);
...
pub(crate) fn get_project_path() -> String {
    load_dotenv(None);
    std::env::var(PROJECT_PATH_ENV).unwrap_or_else(|_| String::new())
}

pub(crate) fn get_test_path() -> String {
    load_dotenv(None);
    std::env::var(TEST_PATH_ENV).unwrap_or_else(|_| String::new())
}
