...
#[cfg(test)]
fn print_digit_matrix<T: std::fmt::Debug>(
    title: &str,
    query: &str,
    reference: &str,
    matrix: &[Vec<T>],
...
pub fn levenshtein_distance(query: &str, reference: &str) -> f32 {
    ...
    for (i, cell) in cmp_matrix.iter_mut().enumerate().take(query_len + 1) {
        cell[i] = i;
    }
    ...
    for i in 1..=query_len {
        for j in 1..=reference_len {
            ...
            cmp_matrix[i][j] = cmp::min(
                cmp::min(cmp_matrix[i - 1][j] + 1, cmp_matrix[i][j - 1] + 1),
...
fn sw_traceback(
    ...
    while i > 0 && j > 0 && cmp_matrix[i][j] > 0 {
        if query_chars[i - 1] == reference_chars[j - 1]
            && cmp_matrix[i][j] == cmp_matrix[i - 1][j - 1] + match_score
        {
