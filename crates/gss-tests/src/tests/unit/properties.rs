...
fn create_properties_file() {
    ...
    assert!(properties_path.ends_with(&format!(
        "{}{}{}",
        CONFIG_DIR,
    ...
    assert_eq!(
        json[PROP_STEAM_API_KEY].as_str().unwrap(),
        "",
        "Expected {} to be empty string in properties file",
        PROP_STEAM_API_KEY
    );
    assert_eq!(
        json[PROP_RECIPIENT_EMAIL].as_str().unwrap(),
        "",
        "Expected {} to be empty string in properties file",
        PROP_RECIPIENT_EMAIL
    );
    assert_eq!(
        json[PROP_SMTP_HOST].as_str().unwrap(),
        "",
        "Expected {} to be empty string in properties file",
        PROP_SMTP_HOST
    );
    assert_eq!(
        json[PROP_SMTP_EMAIL].as_str().unwrap(),
        "",
        "Expected {} to be empty string in properties file",
        PROP_SMTP_EMAIL
    );
    assert_eq!(
        json[PROP_SMTP_USERNAME].as_str().unwrap(),
        "",
        "Expected {} to be empty string in properties file",
        PROP_SMTP_USERNAME
    );
    assert_eq!(
        json[PROP_SMTP_PASSWORD].as_str().unwrap(),
        "",
        "Expected {} to be empty string in properties file",
        PROP_SMTP_PASSWORD
    );
    assert_eq!(
        json[PROP_SMTP_PORT].as_i64().unwrap(),
        0,
        "Expected {} to be 0 in properties file",
        PROP_SMTP_PORT
    );
    assert_eq!(
        json[PROP_PROJECT_PATH].as_str().unwrap(),
        _tmp_env.temp_dir.display().to_string(),
        "Expected {} to be {} in properties file",
        PROP_PROJECT_PATH,
        _tmp_env.temp_dir.display()
    );
    assert_eq!(
        json[PROP_TEST_PATH].as_str().unwrap(),
        _tmp_env
            .temp_dir
            .join(DEFAULT_TEST_DIR)
            .display()
            .to_string(),
        "Expected {} to be {} in properties file",
        PROP_TEST_PATH,
        _tmp_env.temp_dir.join(DEFAULT_TEST_DIR).display()
    );
...
fn load_properties_from_file() {
    ...
    assert_eq!(
        loaded_properties[PROP_STEAM_API_KEY].as_str().unwrap(),
        "",
        "Expected {} to be empty string in loaded properties",
        PROP_STEAM_API_KEY
    );
    assert_eq!(
        loaded_properties[PROP_RECIPIENT_EMAIL].as_str().unwrap(),
        "",
        "Expected {} to be empty string in loaded properties",
        PROP_RECIPIENT_EMAIL
    );
    assert_eq!(
        loaded_properties[PROP_SMTP_HOST].as_str().unwrap(),
        "",
        "Expected {} to be empty string in loaded properties",
        PROP_SMTP_HOST
    );
    ...
    assert_eq!(
        loaded_properties[PROP_SMTP_EMAIL].as_str().unwrap(),
        "",
        "Expected {} to be empty string in loaded properties",
        PROP_SMTP_EMAIL
    );
    assert_eq!(
        loaded_properties[PROP_SMTP_USERNAME].as_str().unwrap(),
        "",
        "Expected {} to be empty string in loaded properties",
        PROP_SMTP_USERNAME
    );
    assert_eq!(
        loaded_properties[PROP_SMTP_PASSWORD].as_str().unwrap(),
        "",
        "Expected {} to be empty string in loaded properties",
        PROP_SMTP_PASSWORD
    );
    assert_eq!(
        loaded_properties[PROP_PROJECT_PATH].as_str().unwrap(),
        _tmp_env.temp_dir.display().to_string()
    );
    assert_eq!(
        loaded_properties[PROP_TEST_PATH].as_str().unwrap(),
        _tmp_env
            .temp_dir
            .join(DEFAULT_TEST_DIR)
            .display()
...
fn sub_directories_created() {
    ...
    assert_eq!(
        data_path,
        _tmp_env.temp_dir.join(DATA_DIR).display().to_string()
    );
    assert_eq!(
        config_path,
        _tmp_env.temp_dir.join(CONFIG_DIR).display().to_string()
    );
...
fn update_properties_from_env() {
    ...
    let mut env_data = file_operations::create_env_str(
        steam_api_key_val,
        recipient_email_val,
        smtp_host_val,
        smtp_port_val,
    ...
    env_data = file_operations::create_env_str(
        steam_api_key_val,
        recipient_email_val,
        smtp_host_val,
        smtp_port_val,
    ...
    let encrypted_steam_key = updated_properties[PROP_STEAM_API_KEY]
        .as_str()
        .unwrap()
        .to_string();
    ...
    assert_eq!(
        updated_properties[PROP_RECIPIENT_EMAIL].as_str().unwrap(),
        recipient_email_val
    );
    assert_eq!(
        updated_properties[PROP_SMTP_EMAIL].as_str().unwrap(),
        smtp_email_val
    );
    assert_eq!(
        updated_properties[PROP_SMTP_USERNAME].as_str().unwrap(),
        smtp_username_val
    );
    assert_eq!(
        updated_properties[PROP_SMTP_HOST].as_str().unwrap(),
        smtp_host_val
    );
    assert_eq!(
        updated_properties[PROP_SMTP_PORT].as_i64().unwrap(),
        smtp_port_val as i64
    );
    assert_eq!(
        updated_properties[PROP_SMTP_HOST].as_str().unwrap(),
        smtp_host_val
    ...
    let encrypted_smtp_pwd = updated_properties[PROP_SMTP_PASSWORD]
        .as_str()
        .unwrap()
        .to_string();
    ...
    assert_eq!(
        updated_properties[PROP_PROJECT_PATH].as_str().unwrap(),
        _tmp_env.temp_dir.display().to_string()
    );
    assert_eq!(
        updated_properties[PROP_TEST_PATH].as_str().unwrap(),
        _tmp_env
            .temp_dir
            .join(DEFAULT_TEST_DIR)
            .display()
