...
fn get_available_stores() {
    ...
    assert!(
        all_stores_valid,
        "One of the follow are not valid: {:?}",
        available_stores
    );
...
fn get_proper_store_name() {
    ...
    assert_eq!(
        "Good Old Games (GOG)", store_name,
        "{} != {}",
        "Good Old Games (GOG)", store_name
    );
    store_name = GameStore::MICROSOFT_STORE_PC.get_name();
    assert_eq!(
        "Microsoft Store (PC)", store_name,
        "{} != {}",
        "Microsoft Store (PC)", store_name
    );
...
fn get_selected_stores() {
    ...
    for store in &selected_stores {
        if store == &GameStore::STEAM {
            is_steam_selected = true
        } else if store == &GameStore::GOOD_OLD_GAMES {
            is_gog_selected = true
        }
        if store == &GameStore::MICROSOFT_STORE_PC {
            is_ms_store_selected = true
        }
    }
    assert!(is_steam_selected, "{} should be selected", GameStore::STEAM);
    assert!(
        is_gog_selected,
        "{} should be selected",
...
fn get_alias_state() {
    ...
    assert!(are_aliases_enabled, "Aliases should be enabled.");
...
fn update_selected_stores() {
    ...
    assert_eq!(
        0,
        selected_stores.len(),
        "No stores should be selected by default"
    );
    ...
    assert_eq!(
        2,
        selected_stores.len(),
        "The number of selected stores should be 2 not {}",
        selected_stores.len()
    );
    assert_eq!(
        GameStore::STEAM,
    ...
    settings::update_selected_stores(vec![
        GameStore::STEAM,
        GameStore::STEAM,
        GameStore::STEAM,
        GameStore::GOOD_OLD_GAMES,
    ...
    for store in store_count {
        ...
        assert_eq!(
            store_limit, *count,
            "\'{}\' should not have more than 1 entry.",
            store.0
        );
    }
...
fn update_alias_state() {
    ...
    assert_eq!(
        DEFAULT_ALIAS_STATE, are_aliases_enabled,
        "Aliases should be enabled by default."
    );
    // Check that aliases are disabled
    settings::update_alias_state(DISABLED_STATE);
    are_aliases_enabled = settings::get_alias_state();
    assert!(!are_aliases_enabled, "Aliases should not be enabled.");
    ...
    for i in -10..10 {
        if i != ENABLED_STATE && i != DISABLED_STATE {
            ...
            assert!(
                !are_aliases_enabled,
                "Aliases should not be enabled given input: {}.",
                i
            );
        }
    }
