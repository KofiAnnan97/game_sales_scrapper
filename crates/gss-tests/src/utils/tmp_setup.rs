...
use constants::operations::properties::{
    CONFIG_DIR, DATA_DIR, DEFAULT_TEST_DIR, PROJECT_PATH_ENV, TEST_PATH_ENV,
};
...
pub fn create(title: &str, steam_cache: Vec<App>) -> PathBuf {
    ...
    let tmp_thresholds_file = data_dir
        .join(THRESHOLD_FILENAME)
        .with_extension("json")
        .display()
        .to_string();
    let tmp_settings_file = config_dir
        .join(SETTINGS_FILENAME)
        .with_extension("json")
        .display()
        .to_string();
    let tmp_steam_cache_file = data_dir
        .join(CACHE_FILENAME)
        .with_extension("json")
        .display()
        .to_string();
    ...
    general::write_to_file(
        tmp_steam_cache_file,
        serde_json::to_string_pretty(&steam_cache)
            .expect("Failed to write steam cache for testing"),
    );
    ...
    let tmp_thresholds_file_test = test_data_dir
        .join(THRESHOLD_FILENAME)
        .with_extension("json")
        .display()
        .to_string();
    let tmp_settings_file_test = test_config_dir
        .join(SETTINGS_FILENAME)
        .with_extension("json")
        .display()
        .to_string();
    let tmp_steam_cache_file_test = test_data_dir
        .join(CACHE_FILENAME)
        .with_extension("json")
        .display()
        .to_string();
...
pub fn clean_up(
    prev_dir: &Path,
    temp_dir: &Path,
    prev_project_path: Option<String>,
    prev_test_path: Option<String>,
) {
